...
function openMyRadio(){location.hash="#/myradio";displayWorking();$.ajax({type:"GET",url:"ajax.php",data:"action=openMyRadio",success:function(a){handleLogoutHtml(a);var b=$("#playercontrols");0<b.size()&&b.remove();$("#musicwrapper").html(a);hideWorking()}})}
function getHomeNavigation(){1>$("#navlist .previousDirectoryListItem .filesize_type .dirlink").size()&&(displayWorking(),$.ajax({type:"GET",url:"ajax.php",data:"action=getHomeNavigation",success:function(a){handleLogoutHtml(a);$("#navlist").html(a);hideWorking()}}))}
...
function init(){var a=window.location.hash,a=a.replace(/^#/,""),a=a.split("/");switch(a[1]){case "open":a=decodeURIComponent(a[2]);openDir(a);break;case "myradio":getHomeNavigation();openMyRadio();break;case "viewstations":getHomeNavigation();viewMyRadio();break;case "loadstation":getHomeNavigation(),a=decodeURIComponent(a[2]),loadRadio(a)}}
