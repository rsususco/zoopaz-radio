...
function(){console.log("bind: Playlist size: "+myPlaylist.playlist.length+", Current position: "+myPlaylist.current)}).bind($.jPlayer.event.ended,function(){myPlaylist.remove(myPlaylist.current-1);$.getJSON("ajax.php?action=getRandomPlaylist&num=1",function(a){$(a).each(function(a,b){myPlaylist.add(b)})})});hideWorking()}})}
...
!0;return!1}function addToPersonalRadio(a,c){var b=a||window.event;displayWorking();var d=$(c).data("dir");$.getJSON("ajax.php?action=addToPersonalRadio&dir="+encodeURIComponent(d),function(a){handleLogoutJson(a);hideWorking()});b.stopPropagation();b.stopImmediatePropagation();b.cancelBubble=!0;return!1}isRadioMode=!1;
...
$(document).on("click",".dirlinkcover",function(){openDir($(this).data("url"))});$(document).on("click",".addtoplaylist",function(a){addToPlaylist(a,this)});$(document).on("click",".jp-clear-playlist",function(a){clearPlaylist(a,this)});$(document).on("click","#logout-link",function(){logout(this)});$(document).on("click","#radio-button",function(a){playRadio(a)});$(document).on("click",".createradiobutton",function(a){createPersonalRadio(a,this)});$(document).on("click",".addtoradiobutton",function(a){addToPersonalRadio(a,
this)});prevtime=parseInt((new Date).getTime());curval="";t=null;$(document).on("keyup","#search",function(){var a=new StreamsConfig;curval=$(this).val();curtime=parseInt((new Date).getTime());next=prevtime+a.searchThreshold;prevtime=curtime;curtime<next&&(clearTimeout(t),t=setTimeout("search('"+curval+"')",a.searchThreshold))});$(".playlist-albumart").livequery(function(){var a=$(this);a.data("done")||$.getJSON("ajax.php?action=getAlbumArt&dir="+encodeURIComponent(a.data("dir"))+"&file="+encodeURIComponent(a.data("file")),
function(c){a.attr("src",c.albumart);a.data("done",!0).css("width","2em").css("height","2em")})})});
