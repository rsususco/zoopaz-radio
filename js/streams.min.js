...
function(){console.log("bind: Playlist size: "+myPlaylist.playlist.length+", Current position: "+myPlaylist.current)}).bind($.jPlayer.event.ended,function(){myPlaylist.remove(myPlaylist.current-1);$.getJSON("ajax.php?action=getRandomPlaylist&num=1",function(a){$(a).each(function(a,b){myPlaylist.add(b)})})});hideWorking()}})}currentMyRadioStation=null;
...
switch(a[1]){case "myradio":playMyRadio(null);case "loadstation":a=decodeURIComponent(a[2]),playMyRadio(a)}});$(document).on("click","#radio-button",function(a){playRadio(a)});$(document).on("click",".createradiobutton",function(a){createPersonalRadio(a,this)});$(document).on("click",".addtoradiobutton",function(a){addToPersonalRadio(a,this)});$(document).on("click",".removefromradiobutton",function(a){removeFromPersonalRadio(a,this)});saveDialog=!1;$(document).on("click","#save-my-radio",function(){clickSaveMyRadio()});
