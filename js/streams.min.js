...
function search(a){if(3>a.length)return!1;0<$("#playbutton").length&&$("#playbutton").parent().remove();displayWorking();$.ajax({type:"GET",url:"ajax.php",data:"action=search&q="+encodeURIComponent(a),success:function(a){handleLogoutHtml(a);$("#musicindex").html(a);hideWorking()}})}function displayWorking(){$("#loading").css("display","block").css("visibility","visible")}function hideWorking(){$("#loading").css("display","none").css("visibility","hidden")}
...
function logout(){displayWorking();$.ajax({type:"GET",url:"ajax.php",data:"action=logout",success:function(){hideWorking();location.href="index.php"}})}function handleLogoutHtml(a){a=JSON.parse(a);handleLogoutJson(a)}function handleLogoutJson(a){a.is_logged_in||logout()}
