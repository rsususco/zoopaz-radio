...
$("#playercontrols").remove(),b=d-16,$("#mediaplayer_wrapper").css("width",b+"px"),b=$("#mediaplayer").offset().top,window.scrollTo(0,b));$("#mediaplayer").bind($.jPlayer.event.play,function(){console.log("bind: Playlist size: "+myPlaylist.playlist.length+", Current position: "+myPlaylist.current)}).bind($.jPlayer.event.ended,function(){console.log("Music has ended, changing tracks.");myPlaylist.remove(myPlaylist.current-1);var a="";null!=currentMyRadioStation&&(a="&station="+currentMyRadioStation);
$.getJSON("ajax.php?action=getRandomPlaylist&num=1&personal=yes"+a,function(a){$(a).each(function(a,b){myPlaylist.add(b)})})});hideWorking()})}
...
function loadRadio(a){location.hash="#/loadstation/"+a;displayWorking();$.getJSON("ajax.php?action=loadStation&station="+encodeURIComponent(a),function(b){"ok"===b.status?($("#musicwrapper").html(b.html),setRadioStationTitle(a)):"error"===b.status?alert(b.message):alert("An unexpected error has occurred. Could not save radio.");handleLogoutJson(b);hideWorking()})}
function setRadioStationTitle(a){var b=$("#radio-station-wrapper");b.html("<div class='album-title'>"+a.replace(/_/," ")+"</div>"+b.html())}function setPlayerStationTitle(a){$("#content-player .album-title").first().html(a.replace(/_/," "))}
